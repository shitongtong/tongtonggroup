...
$(function(){
	$("#checkedAll").change(function(){
        ...
        $("input:checkbox[name='id[]']").each(function(){
            $(this)[0].checked=checkStatus;
        });
    });

    $(":checkbox[name='id[]']").click(function () {
		var all = $(":checkbox[name='id[]']").length;//所有条目的个数
		var select = $("input[name='id[]']:checked").length;//获取所有被选择条目的个数
		   if (all == select) {//全都选中了
		      $("#checkedAll").prop("checked", true);//勾选全选复选框
		   } else {
		      $("#checkedAll").prop("checked", false);//取消全选
		   }
	});
